...
function createHttp(adapter, httpInfo) {
    return new Promise(resolve => {
        try {
            ...
            axios
                .post(`http://${httpInfo.http_ip}/config`, data, { timeout: 30000 })
                .then(res => {
                    if (res && res.data && res.data.payload) {
                        adapter.log.debug(
                            `[createHttp] Request success for ${httpInfo.http_ip}, namespace: ${httpInfo.http_namespace}`,
                        );
                    ...
                    // adapter.log.warn(
                    //     `[createHttp] Invalid response structure for ${httpInfo.http_ip}, namespace: ${httpInfo.http_namespace}`,
                    // );
                    resolve(null);
                })
                .catch(error => {
                    if (error.code === 'ECONNABORTED') {
                        adapter.log.error(
                            `[createHttp] Request timed out for ${httpInfo.http_ip}, namespace: ${httpInfo.http_namespace}`,
                        );
                    } else {
                        adapter.log.error(
                            `[createHttp] Axios Error for ${httpInfo.http_ip}, namespace: ${httpInfo.http_namespace}: ${
                                error.message || error
                            }`,
                        );
                    }
