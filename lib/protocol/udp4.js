...
class BaseServer {
    ...
    constructor(adapter, objectHelper, devices) {
        ...
        this.deviceLastSeen = {}; // 记录每个设备最后响应UDP的时间 {uuid: timestamp}
        this.cleanupCheckInterval = null; // 清理检查定时器

        // Add know devices without message
        for (const device of devices) {
            this.adapter.log.debug(`Add know device ${device['_id']}`);

            const uuid = device['common']['deveiceInfo']['uuid'];
            this.deviceUuidList.push(uuid);
            const bs = new base(adapter, objectHelper, device['common']['deveiceInfo']);
            this.deviceBase.push(bs);
            // 初始化最后响应时间
            this.deviceLastSeen[uuid] = Date.now();
            if (bs.deviceId && typeof this.adapter.registerDeviceId === 'function') {
                this.adapter.registerDeviceId(bs.deviceId);
            }
        }
        this.start();
        ...
        server.on('message', async (msg, rinfo) => {
            ...
            if (msg) {
                ...
                if (this.deviceExists(devName) && !this.deviceUuidList.includes(deveiceInfo.uuid)) {
                    ...
                    // 记录最后响应时间
                    this.deviceLastSeen[deveiceInfo.uuid] = Date.now();
                    ...
                    // 设备已存在，检查是否需要更新IP地址
                    const existingDevice = this.deviceBase.find(device => device.uuid === deveiceInfo.uuid);
                    if (existingDevice) {
                        // 更新最后响应时间
                        this.deviceLastSeen[deveiceInfo.uuid] = Date.now();
                        // 更新IP地址（网络切换后IP可能改变）
                        if (deveiceInfo.ip && deveiceInfo.ip !== existingDevice.ip) {
                            this.adapter.log.info(
                                `[UDP] Updating IP for existing device: ${devName} (UUID: ${deveiceInfo.uuid}) from ${existingDevice.ip} to ${deveiceInfo.ip}`,
                            );
                            existingDevice.updateIP(deveiceInfo.ip);
                        }
                    } else {
                        this.adapter.log.debug(
     ...
     */
    async checkAndCleanupOfflineDevices(timeout) {
        const now = Date.now();
        const devicesToRemove = [];

        for (let i = this.deviceBase.length - 1; i >= 0; i--) {
            const device = this.deviceBase[i];
            const uuid = device.uuid;
            const lastSeen = this.deviceLastSeen[uuid] || 0;
            const timeSinceLastSeen = now - lastSeen;

            // 检查设备是否离线且长时间未响应
            const isOffline = !this.adapter.isOnline(device.deviceId);
            if (isOffline && timeSinceLastSeen > timeout) {
                this.adapter.log.info(
                    `[UDP] Removing offline device: ${device.deviceId} (UUID: ${uuid}), last seen: ${Math.round(timeSinceLastSeen / 1000 / 60)} minutes ago`,
                );
                devicesToRemove.push({ device, index: i, uuid });
            }
        }

        // 清理设备
        /*
        for (const { device, index, uuid } of devicesToRemove) {
            await device.destroy();
            this.deviceBase.splice(index, 1);
            const uuidIndex = this.deviceUuidList.indexOf(uuid);
            if (uuidIndex !== -1) {
                this.deviceUuidList.splice(uuidIndex, 1);
            }
            delete this.deviceLastSeen[uuid];
        }
        */
    }
    /**
    ...
    async destroy() {
        ...
        for (const i in this.deviceBase) {
            await this.deviceBase[i].destroy();
        }
        server.close();
    }
}
